...
output = f'7nn0_filtered_mols_1_{now}.csv'
...
for candidate_num in range(len(mydata)):
    ...
    try: 
        ...
        v.set_receptor('7nn0_OK_A.pdbqt')
        ...
        previous_ligand = next(rdkit.Chem.SDMolSupplier('pocket1_7nn0.sdf', sanitize = False))
        ...
        with open(output, 'a') as file:
                file.write(f'{mydata["eos"][candidate_num]},{v.score()[0]}\n')
    except:
        with open(output, 'a') as file:
            file.write(f'{mydata["eos"][candidate_num]},ERROR\n')
#df = pd.DataFrame(scores_pocket1_7nn0, columns=["molecule", "score"])
#df.to_csv('scores_pocket1_7nn0.csv', index=False) 
