...
def docking(protein_file, ligand, pocket, mode):
    ...
    v.compute_vina_maps(center=[-25.229235, 25.126171, -28.270075], 
                       box_size=[30, 30, 30])
...
def docking_pipeline(molecules, protein, pocket, mode='screening', start=0):
    ...
    # Folder where the molecules file lies
    molecules_folder, molecules_file = os.path.split(molecules)
    ...
    if mode == 'screening':
        output_folder = os.path.join(molecules_folder, f'docking_{now}')
    elif mode == 'refinement':
        output_folder = os.path.join(molecules_folder, f'refinement_docking_{now}')
    os.makedirs(output_folder, exist_ok = True)
    output_file = output_folder + f'/{molecules_file[:-4]}_{now}.csv'
...
if __name__=='__main__':
    molecules = "../docking/7NIO_pocket1/7NIO_pocket1_docking_results_merged_filtered_-11.csv"
    protein = "../docking/7nn0_OK_A.pdbqt"
    pocket = '../docking/pocket1_7nn0.sdf'
    docking_pipeline(molecules, protein, pocket,
                     mode = 'refinement', start = 0)